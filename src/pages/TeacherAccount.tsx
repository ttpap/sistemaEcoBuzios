...
import { DEFAULT_TEACHER_PASSWORD, readGlobalTeachers, updateGlobalTeacher, resetTeacherPasswordToDefault } from "@/utils/teachers";
import { ArrowLeft, KeyRound, Save, RotateCcw, ShieldCheck } from "lucide-react";
...
export default function TeacherAccount() {
  ...
  const teacher = useMemo(() => {
    ...
    return readGlobalTeachers([]).find((t) => t.id === teacherId) || null;
  }, [teacherId]);
  ...
  const onSave = () => {
    ...
    updateGlobalTeacher(teacherId, { authPassword: pw1 });
    setPw1("");
    setPw2("");
  ...
  const onReset = () => {
    if (!teacherId) return;
    const ok = window.confirm(`Resetar sua senha para a senha padrão (${DEFAULT_TEACHER_PASSWORD})?`);
    if (!ok) return;
      ...
      <Card className="border-none shadow-xl shadow-slate-200/40 bg-white rounded-[2.5rem] overflow-hidden">
        <CardHeader className="p-6 md:p-8 bg-primary text-white">
          ...
          <p className="mt-1 text-white/85 text-sm font-bold">
            {teacher ? `Olá, ${teacher.fullName}.` : "Gerencie sua senha."}
          </p>
        </CardHeader>
