...
const API_BASE = String(import.meta.env.VITE_SUPABASE_URL || "").replace(/\/$/, "");
      ...
      <Card className="border-none shadow-xl shadow-slate-200/50 bg-white rounded-[2.5rem] overflow-hidden">
        <CardContent className="p-8 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-black text-slate-800">Endpoints disponíveis</h2>
          </div>
          <p className="text-sm font-medium text-slate-500">
            Envie o header <code className="bg-slate-100 rounded px-1 py-0.5 text-xs font-black">x-api-key</code> com uma chave ativa em cada requisição (método GET).
          </p>

          {[
            {
              nome: "Editais",
              desc: "Lista os editais registrados (código, nome, título, situação, status). Filtro opcional: ?status=aprovado | reprovado | inscrito",
              url: `${API_BASE}/functions/v1/editais-api`,
            },
            {
              nome: "Estatísticas gerais",
              desc: "Painel de alunos, frequência, horas e resumo de editais. Filtro opcional: ?projetos=id1,id2",
              url: `${API_BASE}/functions/v1/public-stats-api`,
            },
          ].map((ep) => (
            <div key={ep.nome} className="rounded-2xl border border-slate-100 bg-slate-50/60 p-4">
              <div className="flex items-center justify-between gap-3">
                <p className="font-black text-slate-800">{ep.nome}</p>
                <Button
                  size="sm"
                  variant="outline"
                  className="rounded-xl font-black h-8"
                  onClick={() => {
                    void copyToClipboard(ep.url);
              ...
              <p className="text-xs font-medium text-slate-500 mt-1">{ep.desc}</p>
              <code className="mt-2 block break-all rounded-lg bg-white border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700">
                {ep.url}
              </code>
            </div>
          ))}
