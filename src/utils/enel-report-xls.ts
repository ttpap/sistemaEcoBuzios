 ...
 */
export async function downloadEnelXls(input: { month: string; rows: EnelRow[] }) {
  const project = getActiveProject();
  const projectName = project?.name || "Projeto";
  const title = "Relatório ENEL";
  const subtitle = monthLabel(input.month);
  ...
  const filename = filenameSafe(`relatorio-enel-${projectName}-${input.month}.xls`);
