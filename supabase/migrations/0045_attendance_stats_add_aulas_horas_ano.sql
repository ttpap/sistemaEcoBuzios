-- Adiciona total_aulas_ano e total_horas_ano ao retorno da função.
-- Duração por aula = end_time - start_time (texto HH:MM); fallback 2h.
-- Lógica igual ao frontend (Reports.tsx parseTimeHours).
...
BEGIN
  -- ── Sessões do ano com duração calculada ──────────────────────────────────
  WITH sessoes_dur AS (
    SELECT
      s.id AS session_id,
      s.date,
  ...
  -- ── Totais de aulas e horas ───────────────────────────────────────────────
  SELECT
    COUNT(*)::int,
    ROUND(SUM(duracao_horas)::numeric, 1)
  INTO v_total_aulas, v_total_horas
  FROM sessoes_dur;

  SELECT jsonb_build_object(
    'ano',             v_ano,
    'total_aulas_ano', v_total_aulas,
    'total_horas_ano', v_total_horas,
    'mensal', COALESCE(
      (
        SELECT jsonb_agg(
          jsonb_build_object(
